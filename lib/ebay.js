...
export async function fetchEbaySoldListings(query, category = null, limit = 50) {
  try {
    const appId = process.env.EBAY_APP_ID
    if (!appId) {
      console.error('eBay Finding API: EBAY_APP_ID not configured')
      return emptyPriceResult('ebay_sold')
    }
    ...
    if (!response.ok) {
      console.error('eBay Finding API error:', await response.text())
      return emptyPriceResult('ebay_sold')
    }
    ...
    console.error('eBay Finding API fetch error:', error)
    return emptyPriceResult('ebay_sold')
  }
}
