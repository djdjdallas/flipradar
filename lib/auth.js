/**
 * Centralized authentication helper for FlipRadar API
 ...
 * Supports both Supabase JWT tokens and FlipRadar API keys
 *
 * @param {Request} request - The incoming request
 * @returns {Promise<{user, profile, authMethod} | {error, status}>}
 */
export async function authenticateBearer(request) {
  ...
  // Check if it's a FlipRadar API key
...
/**
 * Authenticate using FlipRadar API key
 *
 * @param {string} apiKey
 * @returns {Promise<{user, profile, authMethod} | {error, status}>}
 */
async function authenticateWithApiKey(apiKey) {
  try {
    ...
    if (error || !profile) {
      console.log('[FlipRadar API] Invalid API key attempt')
    ...
    console.error('[FlipRadar API] API key auth error:', err)
...
async function authenticateWithSession() {
  try {
    ...
    console.error('[FlipRadar API] Session auth error:', err)
    return { error: 'Authentication failed', status: 500 }
  }
}
...
async function authenticateWithJwt(token) {
  try {
    ...
    console.error('[FlipRadar API] JWT auth error:', err)
    return { error: 'Authentication failed', status: 500 }
  }
}
